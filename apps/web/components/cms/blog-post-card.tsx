...
export function BlogPostCard({ post, featured = false }: BlogPostCardProps) {
  ...
  const readLabel = locale === "ar" ? "اقرأ التحليل" : locale === "ru" ? "Читать анализ" : "Read analysis"
        ...
        <CardContent className="card-stack justify-center p-7 md:p-8">
          ...
          <div className="card-stack">
            <h3
              className={
                featured
                  ? "max-w-[17ch] text-[1.9rem] leading-[1.1] text-foreground md:text-[2.35rem]"
                  : "max-w-[18ch] text-[1.58rem] leading-[1.15] text-foreground"
              }
            >
              {post.title}
            </h3>
