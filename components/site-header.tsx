      ...
      <div className="container-shell py-3 md:py-3.5">
        <div className="flex flex-col gap-3">
          <div className={cn("flex items-center justify-between gap-6", isRtl && "flex-row-reverse")}>
            <BrandMark />
            <nav className={cn("hidden items-center gap-5 md:flex", isRtl && "md:flex-row-reverse")}>
              {mainNavLinks.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={cn("text-sm font-medium text-muted-foreground/90 hover:text-foreground", isRtl && "text-right")}
                >
                  {item.label}
                </Link>
              ))}
